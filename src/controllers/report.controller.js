...
const createReport = catchAsync(async (req, res) => {
  ...
  if (report) {
    ...
    const newVersion = {
      ...
      summary: summary || 'Report generated by Sitewise',
    ...
    report = await Report.create({
      ...
      versions: [{
        ...
        summary: summary || 'Initial inspection report generated by Sitewise',
        generatedBy: user._id,
        watermark: true,
      }],
    });
  ...
  const targetVersion = report.currentVersion;
  ...
  }, 'Report record created and PDF generated');
});
 ...
 * @returns {Promise<{ downloadUrl: string, generatedAt: Date, expiresAt: Date }>}
 */
const generateAndUploadReportPDF = async ({
  report,
  inspection,
  user,
  targetVersion,
  ...
  const organization = { 
    ...
    name: user.name ? `${user.name}'s Organization` : 'Sitewise',
  };
  ...
  logger.info({ reportId: report._id, pdfSize: pdfBuffer?.length }, 'PDF buffer generated');
  ...
  try {
    ...
    downloadUrl = await storage.getPresignedDownloadUrl(pdfPath, {
      expiresIn: 3600,
      responseContentDisposition: `attachment; filename="inspection-report-${report._id}-v${targetVersion}.pdf"`,
    });
  ...
  return { downloadUrl, generatedAt, expiresAt };
};
...
const generateReportPDF = catchAsync(async (req, res) => {
  ...
  const { downloadUrl, generatedAt, expiresAt } = await generateAndUploadReportPDF({
    report,
    inspection,
    user,
    targetVersion,
