 ...
 * @property {string} [text] - Watermark text (default: 'TRIAL - InspectAI')
...
const addWatermark = (doc, watermark) => {
  ...
  const text = watermark.text || 'TRIAL - InspectAI';
...
const generateInspectionReportPDF = async ({
  ...
  const watermark = {
    enabled: isTrialUser,
    text: 'TRIAL VERSION - InspectAI.com',
    opacity: 0.1,
  };
  ...
  return generatePDF({
    ...
    sections,
    footer: isTrialUser 
      ? 'Trial Version - Upgrade at InspectAI.com for branded reports'
